...
{distances, queue} =
  ...
  Enum.reduce(neighbors, {distances, queue}, fn {neighbor, cost}, {distances, queue} ->
    ...
    # Controleer de huidige geregistreerde afstand naar deze buur
    case distances[neighbor] do
      # Als de buur nog niet eerder is bezocht
      @infinity ->
        # Update de afstand en voeg de buur toe aan de queue
        {Map.put(distances, neighbor, new_dist), [{new_dist, neighbor} | queue]}

      # Als er een korter pad hebben gevonden naar deze buur
      current_dist when new_dist < current_dist ->
        # Update de afstand en voeg opnieuw toe aan de queue
        {Map.put(distances, neighbor, new_dist), [{new_dist, neighbor} | queue]}

      # Het bestaande pad is al korter of gelijk, geen actie nodig
      _ ->
        {distances, queue}
    end
  end)
